...
> {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
        ...
        this.dataSource = [
          {
            ...affiliateCreated,
            relationshipName: affiliateCreated.relationship.name,
            affiliator:
              affiliateCreated.user.lastName +
              " " +
              affiliateCreated.user.firstName,
            age: getAge(affiliateCreated.birthDate),
          },
          ...this.dataSource,
        ];
      }
    ...
    dialogRef.afterClosed().subscribe((result) => {
      ...
      if (result) {
        ...
        const affiliateUpdated = {
          ...affiliateToUpdate,
          relationshipName: affiliateToUpdate.relationship.name,
          affiliator:
            affiliateToUpdate.user.lastName +
            " " +
            affiliateToUpdate.user.firstName,
          age: getAge(affiliateToUpdate.birthDate),
        };
        this.dataSource = this.dataSource.map((affiliate) =>
          affiliate.dni === elem.dni ? affiliateUpdated : affiliate
        );
      }
    });
  }
  ...
  private processModelList(modelList: Affiliate[]): Affiliate[] {
    return modelList.map((affiliate) => ({
      ...
      deceased: !!affiliate.deceased ? "Si" : "No",
    }));
  }
}
