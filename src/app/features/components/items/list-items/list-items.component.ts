...
> {
  ...
  override getModelList(): void {
    this.service
      ...
      .subscribe({
        next: (modeltList) => {
          this.dataSource = modeltList.map((item) => ({
            ...item,
  ...
  override updateElement(elem: Item): void {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
        if (
          result?.data?.category?.name.toLowerCase() !== ("ataud" || "ataúd")
        ) {
        ...
        this.dataSource = this.dataSource.map((cat) =>
          cat.code === elem.code ? itemToUpdate : cat
        );
      }
    });
  }
